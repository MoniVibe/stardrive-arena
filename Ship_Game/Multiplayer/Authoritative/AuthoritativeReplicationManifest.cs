...
public static class AuthoritativeReplicationManifest
{
    static readonly ReplicatedRowDescriptor[] Rows =
    {
        ...
        new("XS.SystemExploration", "XS", "Exploration", "SystemExploredMasks",
            AuthoritativeReplicationApplyMode.DirectReplay, AuthoritativeReplicationDigestPolicy.Fatal,
            emit: AuthoritativeStateSnapshot.EmitSystemExplorationRows, apply: null,
            Fields("SystemId", "ExploredByMask", "FullyExploredByMask"),
            Fields("SystemId", "ExploredByMask", "FullyExploredByMask"),
            "Host-authored per-empire system exploration and fully-explored masks for passive clients, which do not run local exploration simulation.",
            emitStage: AuthoritativeReplicationEmitStage.PreScoped,
            initialApplyLine: AuthoritativeStateSnapshot.ApplySystemExplorationLine),
        ...
        new("XP.PlanetExploration", "XP", "Exploration", "PlanetExploredMasks",
            AuthoritativeReplicationApplyMode.DirectReplay, AuthoritativeReplicationDigestPolicy.Fatal,
            emit: null, apply: null,
            Fields("PlanetId", "ExploredByMask"), Fields("PlanetId", "ExploredByMask"),
            "Host-authored per-empire planet exploration masks for passive clients, which do not run local exploration simulation.",
            emitStage: AuthoritativeReplicationEmitStage.PerPlanet,
            emitPlanet: AuthoritativeStateSnapshot.EmitPlanetExplorationRows,
            initialApplyLine: AuthoritativeStateSnapshot.ApplyPlanetExplorationLine),
        ...
        new("WF.WeaponFire", "WF", "WeaponFire", "TransientFireEvents",
            AuthoritativeReplicationApplyMode.DirectReplay, AuthoritativeReplicationDigestPolicy.Transform,
            emit: AuthoritativeStateSnapshot.EmitWeaponFireRows, apply: null,
            Fields("Tick", "Sequence", "ShooterShipId", "WeaponUid", "ModuleGridX", "ModuleGridY",
                "TargetShipId", "Source.X", "Source.Y", "Direction.X", "Direction.Y",
                "IsBeam", "Destination.X", "Destination.Y"),
            Fields("Tick", "Sequence", "ShooterShipId", "WeaponUid", "ModuleGridX", "ModuleGridY",
                "TargetShipId", "Source.X", "Source.Y", "Direction.X", "Direction.Y",
                "IsBeam", "Destination.X", "Destination.Y", "RenderOnlyVisualSpawn"),
            "Transient host-authored weapon-fire events for passive-client cosmetic projectiles/beams; excluded from SyncDigest and covered by TransformDigest for same-snapshot delivery.",
            emitStage: AuthoritativeReplicationEmitStage.PostScoped,
            initialApplyLine: AuthoritativeStateSnapshot.ApplyWeaponFireLine),
